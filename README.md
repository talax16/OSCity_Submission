...
Examiners do not need to clone this repository, build the plugin, or run a local server. The OSCity world will be hosted on separate Paper servers.
...
1. Open Minecraft Java Edition `1.21.11`.
2. Click **Multiplayer**.
3. Click **Add Server**.
...
Separate server addresses are provided so each examiner has an isolated session database. The hosted servers use `online-mode=false` so examiner activity is not tied to personal Minecraft/Microsoft accounts.
